...
const AnalysisRequest = () => {
  ...
  const handleSubmit = async (selectedMode: "free" | "premium") => {
    ...
    try {
      ...
      const requestDoc = await addDoc(collection(db, "userRequests"), {
        userId: user.uid,
        text: requestText,
        images: imageUrls,
        createdAt: new Date(),
      ...
      const unsubscribe = onSnapshot(doc(db, "userRequests", requestDoc.id), (doc) => {
        const data = doc.data();
        if (data?.response) {
          ...
          <Card className="p-6">
            ...
            <div className="space-y-6">
              <Textarea
                ...
                placeholder="Décrivez votre demande d'analyse... (ex: Analyse technique de l'EUR/USD, Pronostic pour le match PSG vs Real Madrid)"
          ...
          <Card className="p-6">
            ...
            {!botResponse ? (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p>{isLoading ? "En attente de la réponse de l'admin..." : "Aucune analyse disponible"}</p>
                <p className="text-sm mt-2">
                  {isLoading 
                    ? "L'admin traitera votre requête dès que possible" 
                    : "Envoyez une requête pour obtenir une analyse"}
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                {/* Asset Header */}
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold text-foreground">{botResponse.asset}</h2>
                  <Badge variant="secondary" className="text-lg px-4 py-2">
                    {botResponse.signals.includes("ACHAT") || botResponse.signals.includes("BUY") ? (
                ...
                <div>
                  ...
                  <p className="text-foreground leading-relaxed whitespace-pre-wrap">
                    {botResponse.technicalAnalysis}
                  </p>
                </div>
                
                {/* Analyse Fondamentale */}
                {botResponse.fundamentalAnalysis && (
                  <div>
                ...
                <div className="bg-gradient-subtle p-4 rounded-lg">
                  <h3 className="text-xl font-semibold mb-3 text-accent">Setup de Trading</h3>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <Badge variant="outline">Signal:</Badge>
                      <span className="font-semibold text-lg">{botResponse.signals}</span>
                    </div>
